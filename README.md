...
**Safety gates.** A step that involves power will not advance on "next". The washer's drain pump filter and both furnace procedures open with a gate: the walkthrough waits until the person says the machine is off before moving on. Nobody gets talked into reaching inside a live appliance because they said "next" out of habit.

A gate is cleared by what the person said about the machine, checked on the server, not by anyone asking for it to be cleared. "It's unplugged" clears it; "yes" does not, because agreeing to a repair is not the same as confirming the power is off. This exists because the model, asked "want me to walk you through it?" and told "yes please", called the tool that clears gates. Seven tests pin the wording and twenty-three pin the behaviour.

**Who a job belongs to.** Every maintenance task records whether it is the homeowner's or a dealer's, because the manuals are explicit about it: Carrier says untrained people may change filters and "all other operations must be performed by trained service personnel". Fifteen of the furnace's seventeen tasks are a dealer's, and a due list must not read them out in the same voice as changing a filter.
